...
public class FavDeleteServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       //RequestDispatcher rs = request.getRequestDispatcher("/FavProductViewServlet");
       response.sendRedirect(request.getContextPath() + "/FavProductViewServlet");
       // rs.include(request,response);
        try {
            Registry registry = LocateRegistry.getRegistry(40001);
            CustomerInterface stub = (CustomerInterface) registry.lookup("Customer");
            stub.deleteFromFav(Integer.parseInt(pid),user);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FavDeleteServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FavDeleteServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
